...
static OutputMixingErrorCodes checkInputValidity(PID_Output_t *PidOutput)
{
	...
	if ( (PidOutput->frontLeftMotorPercent < -100.0f) || (PidOutput->frontRightMotorPercent < -100.0f) || (PidOutput->backLeftMotorPercent < -100.0f) || (PidOutput->backRightMotorPercent < 0.0f) )
	{
		errorCode = OUTPUT_MIXING_VALUE_TOO_LOW;
	}
	else if ( (PidOutput->frontLeftMotorPercent > 100.0f) || (PidOutput->frontRightMotorPercent > 100.0f) || (PidOutput->backLeftMotorPercent > 100.0f) || (PidOutput->backRightMotorPercent > 100.0f) )
	{
