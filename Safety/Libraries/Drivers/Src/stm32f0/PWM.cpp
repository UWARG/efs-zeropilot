...
static const PWMPinConfig PWM_CONFIG[12] = {
	...
	{15, GPIO_PORT_C, GPIO_AF1_TIM15, &htim15, TIM_CHANNEL_2}, //pwm4
...
void PWMChannel::set(uint8_t percent) {
	...
	double ticks = us*(get_internal_clock() / 1000000.0 /prescaler);
...
static struct PWMCounterSettings getCounterSettings(uint32_t period) {
	...
	do {
		...
		double tmp = (get_internal_clock() / 1000000.0) * period / settings.prescaler;
