//
// Created by antho on 10/3/2021.
//

#include <gtest/gtest.h>

#include "crc_Mock.hpp"

using namespace std;
using ::testing::Test;


static unsigned char auchCRCHi[] = {
        };

static char auchCRCLo[] = {
        };


// CRC calculation
uint16_t crc_calc_modbus(const int16_t msgBuffer[], size_t len) {
    {
        uint8_t low_byte_crc = 0xFF;  /* low byte of CRC initialized */
        uint8_t high_byte_crc = 0xFF; /* high byte of CRC initialized */
        uint8_t uIndex = 0;           /* will index into CRC lookup table */
        while (len--)                 /* pass through message buffer */
        {
            uIndex = high_byte_crc ^ *msgBuffer++; /* calculate the CRC */
            high_byte_crc = low_byte_crc ^ auchCRCHi[uIndex];
            low_byte_crc = auchCRCLo[uIndex];
        }
        return (high_byte_crc << 8) | low_byte_crc;
    }
}

// test correct CRC calculation for different arrays.
TEST(CRCTest, CRCCalculation) {
    /***SETUP***/
    int16_t test_min_pwm[12];
    for (int i = 0; i < 12; i++) {
        test_min_pwm[i] = INT16_MIN;
    }
    uint16_t test_min = crc_calc_modbus(test_min_pwm, 12);

    int16_t test_max_pwm[12];
    for (int i = 0; i < 12; i++) {
        test_max_pwm[i] = INT16_MAX;
    }
    uint16_t test_max = crc_calc_modbus(test_max_pwm, 12);

    int16_t test_zero_pwm[12];
    for (int i = 0; i < 12; i++) {
        test_zero_pwm[i] = 0;
    }
    uint16_t test_zero = crc_calc_modbus(test_zero_pwm, 12);

    int16_t test_negative_pwm[12];
    for (int i = 0; i < 12; i++) {
        test_negative_pwm[i] = -INT16_MAX;
    }
    uint16_t test_negative = crc_calc_modbus(test_negative_pwm, 12);

    int16_t test_ascending_pwm[12];
    for (int i = 0; i < 12; i++) {
        test_ascending_crc[i] = i;
    }
    uint16_t test_ascending = crc_calc_modbus(test_ascending_pwm, 12);

    /***ASSERTS***/
    /* calculated by hand */
    EXPECT_EQ(test_min, 25602);
    EXPECT_EQ(test_max, 12675);
    EXPECT_EQ(test_zero, 25602);
    EXPECT_EQ(test_negative, 48153);
    EXPECT_EQ(test_ascending, 41364);
}

// check that CRC is being sent properly


// TODO: tests for safety
// - Make sure CRC is being received?
// - Verify that CRC is being calculated
// - Verify that CRC is being cross-checked
// - Verify that we will get kicked out if CRC is incorrect X times in a row